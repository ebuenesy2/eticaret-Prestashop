...
console.log("admin / adminEdit.js");
    ...
    if(userName == '') { 
        ...
        $.ajax({
            ...
            data: {
                ...
                id:Number($('#adminInfo').attr('adminId')),
                ...
                updated_byId: document.cookie.split(';').find((row) => row.startsWith(' yildirimdev_adminId='))?.split('=')[1]
            },
        ...
        if (result.isConfirmed) {
            ...
            $.ajax({
                ...
                data: {
                    ...
                    id:Number(data_id),
                    //created_byId: document.cookie.split(';').find((row) => row.startsWith(' yildirimdev_adminId='))?.split('=')[1],
                },              
                beforeSend: function() { console.log("Başlangıc"); },
                success: function (response) {
                    //alert("başarılı");
...
$("html").keydown(function(event) {
    ...
    if (event.keyCode == 46) {
        var data_id = $('#adminInfo').attr('adminId');
        deleteItem(data_id); //! sil
    }
});
//! ************ Tuş Algılama Son ***************
